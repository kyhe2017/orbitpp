module class_distribution
  ...
  type, public :: distribution
     ...
     real*8, dimension(:), allocatable :: &
          & pol, thet, zet, xx, zz, &
          & vpar, vper, vr, &
          & en, rmu, tm, ptch, wegtF, &
          & enloss, ptchloss, zloss, &
          & philoss, xloss, thetloss, &
          & timeloss
   contains
     procedure, pass :: init
...
contains
  ...
  subroutine ld_data(self, fid)
    ...
    do k = 1, self%nlive
       read(fid, '(3I15,18E15.6)') &
            ...
            & self%en(k), self%rmu(k), self%tm(k), self%ptch(k), self%wegtF(k), &
            & self%enloss(k), self%ptchloss(k), self%zloss(k), &
            & self%philoss(k), self%xloss(k), self%thetloss(k), &
            & self%timeloss(k)
    end do
    return
  ...
  subroutine wrt3d(self, str1, str2, str3)
    ...
    tmpstr = 'ds3_'//trim(str1)//'_'//trim(str2)//'_'//trim(str3)
    if (self%time <= 9) then
       write(numstr, '(A1,I1,A1,I1)') '_', self%ltag, '_', self%time
